...
void i2c_slave_handler(void) {
    unsigned char data;
    IODIR0 |= 1 << 6; // Set P0.6 as output (for LED or indicator)
    ...
    switch (I2STAT) {
        ...
        case 0x80:  // Data byte received
            ...
            lcd_data(data);    // Display received byte on LCD
...
int main(void) {
    ...
    while (1) {
        if (I2CONSET & 0x08) { // Check if SI flag is set (event occurred)
